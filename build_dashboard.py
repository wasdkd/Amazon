...
def build_dashboard():
    ...
    for f in files:
        ...
        key = "".join(c for c in fname if c.isalnum() or c in (' ', '.', '_')).rstrip().replace(" ", "_").replace(".", "_")
    ...
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    window.uploadFile = function (){{
      ...
      inp.onchange = async (e) => {{
        ...
        try{{ await GH.callGitHub("PUT", path, {{message:`upload ${{file.name}}`, content, branch:GH.BRANCH}}, tok);
